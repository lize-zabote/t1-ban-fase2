...
class FuncionarioModel {
    ...
    static void update(FuncionarioBean f, MongoDatabase db) throws Exception {
        UpdateResult result = db.getCollection("funcionario").updateOne(Filters.eq("id", f.getIdFuncionario()),
            Updates.combine(
        ...
        if (result.getMatchedCount() == 0) {
            throw new Exception("Funcionário com ID " + f.getIdFuncionario() + " não encontrado.");
        }
    }
    ...
    static void remove(int id, MongoDatabase db) throws Exception {
        ...
        DeleteResult result = db.getCollection("funcionario").deleteOne(Filters.eq("id", id));
        if (result.getDeletedCount() == 0) {
            throw new Exception("Funcionário com ID " + id + " não encontrado para exclusão.");
        }
    }
}
